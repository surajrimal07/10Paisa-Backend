...
export default async function initializeTokenManager() {
...
export const fetchHeaders = await getHeaders();
...
export const singleFetch = async (url, reqType) => {
  ...
  if (reqType) {
    ...
    if (reqType === "APOST") {
      ...
      // else means this is requesting floorsheet data where we need to loop through the pages and merge the data
      body = (await preparePostRequestFloorsheet()).body;
    }

    console.log("floorsheet chunk body:", body);
    console.log("floorsheet chunk url:", baseURL + url);
    console.log("floorsheet chunk headers:", fetchHeaders);
...
export async function preparePostRequestIndex() {
  ...
  const postPayloadId =
    ...
    (e.id % 10 < 5
      ? tokenManagerInstance.salts[2]
      : tokenManagerInstance.salts[0]);
...
//functional
export async function get_NepseCompanyPriceVolumeHistory(
  symbol,
  startDate,
  endDate
) {
  ...
  return await singleFetch(
    `/api/nots/market/history/security/${id}?&size=500&startDate=${startDate}&endDate=${endDate}`
  );
}
...
export async function get_NepseCompanySupplyDemand(symbol) {
  return await singleFetch(
    `https://www.nepalstock.com/api/nots/api/nots/nepse-data/supplydemand/${symbol}`
  );
}
...
//working if errors then make sure that day nepse opened
export async function get_NepseCompanyFloorsheet(
  symbol = "NABIL",
  business_date = null
) {
  const id = allCompanies[symbol];
...
export async function get_NepseFloorsheet() {
  ...
  while (pageNumber < totalPages) {
    ...
    try {
      ...
      if (
        response &&
        response.floorsheets &&
        response.floorsheets.content.length > 0
      ) {
...
export const fetchFloorsheetsInChunkss = async (startPageNumber = 0) => {
  const pageSize = 500;
  ...
  while (hasMoreData && pageNumber - startPageNumber < totalPagesToFetch) {
    ...
    console.log("chunk fetch url:", url);

    const response = await singleFetch(url, "FPOST");

    console.log("chunk fetch response:", response);
  ...
  console.log("Fetched data till page to", pageNumber);

  console.log("Total data fetched:", allFloorsheets.length);
...
//working
export async function get_NepseSectorwiseSummary() {
  //of today
