...
const filePath = path.join(parentDir, "SymbolNameMap.json");
...
export async function UpdateNameSymbolMapJSON() {
  try {
    let symbolNameMap = await fetch(
      "https://www.nepsealpha.com/trading/1/search?limit=500&query=",
      {
        headers: {
          ...
          "sec-ch-ua-full-version-list":
            '"Microsoft Edge";v="125.0.2535.79", "Chromium";v="125.0.6422.112", "Not.A/Brand";v="24.0.0.0"',
          "sec-ch-ua-mobile": "?0",
          "sec-ch-ua-model": '""',
  ...
  try {
    ...
    const response = await fetch("https://www.sharesansar.com/live-trading", {
      headers: {
        ...
        cookie:
          "_ga=GA1.1.226892968.1702885063; _ga_FPXFBP3MT9=GS1.1.1702885062.1.1.1702886988.60.0.0; XSRF-TOKEN=%3D; sharesansar_session=%3D",
      },
    ...
    nepseIndexes.forEach(({ field, symbol, name }) => {
      ...
      if ($element.length) {
        stockIndexData[field] = {
          ...
          name: name,
          volume:
            parseInt($element.find(".mu-price").text().replace(/,/g, ""), 10) ||
            0,
          index:
            parseFloat($element.find(".mu-value").text().replace(/,/g, "")) ||
            0,
          percent:
            parseFloat($element.find(".mu-percent").text().replace(/%/g, "")) ||
            0,
        };
      }
    });
...
export async function FetchSingleCompanyDatafromAPI(symbol) {
  ...
  try {
    ...
    assetLogger.error(
      `Error at FetchSingleCompanyDatafromAPI : ${error.message}`
    );
    return null;
  }
}
...
async function sendSupplyDemandNotification(modifiedItem) {
  ...
  if (recentlyNotifiedSymbols.has(modifiedItem.symbol)) {
    nepseLogger.info(
      `Notification for ${modifiedItem.symbol} skipped due to recent notification.`
    );
    return;
  }

  if (
    modifiedItem.buyToSellOrderRatio > 7 ||
    modifiedItem.buyToSellQuantityRatio > 7
  ) {
    let ratioComparison = "";
    const title = `Bulk Orders at ${modifiedItem.symbol}`;

    if (modifiedItem.buyToSellOrderRatio > 1) {
      ratioComparison += `Buy order is ${modifiedItem.buyToSellOrderRatio.toFixed(
        1
      )} times higher than sell order and `;
    } else {
      ratioComparison += `Sell order is ${(
        1 / modifiedItem.buyToSellOrderRatio
      ).toFixed(1)} times higher than buy order and `;
    }

    if (modifiedItem.buyToSellQuantityRatio > 1) {
      ratioComparison += `buy quantity is ${modifiedItem.buyToSellQuantityRatio.toFixed(
        1
      )} times higher than sell quantity`;
    } else {
      ratioComparison += `sell quantity is ${(
        1 / modifiedItem.buyToSellQuantityRatio
      ).toFixed(1)} times higher than buy quantity`;
    }
...
const nepseNotification =
  // eslint-disable-next-line no-undef
  process.env.IS_NEPSE_NOTIFICATION_ENABLED === "true" ? true : false;
...
const sendNotificationsIfNeeded = async (currentHighestQuantityPerOrder) => {
  const previousHighestQuantityPerOrder = await fetchFromCache(
    "previousHighestQuantityPerOrder"
  );
  ...
  if (
    !previousHighestQuantityPerOrder ||
    previousHighestQuantityPerOrder.length === 0
  ) {
    nepseLogger.info(
      "No previous data found, treating all items as significant change."
    );
    await saveToCache(
      "previousHighestQuantityPerOrder",
      currentHighestQuantityPerOrder
    );

    if (nepseNotification) {
      for (const item of currentHighestQuantityPerOrder) {
        nepseLogger.info(
          `Notification triggered for new symbol: ${item.symbol}`
        );
  ...
  const previousMap = new Map(
    previousHighestQuantityPerOrder.map((item) => [item.symbol, item])
  );
  ...
  for (const currentItem of currentHighestQuantityPerOrder) {
    ...
    if (!previousItem) {
      nepseLogger.info(
        `Significant change detected for new symbol: ${currentItem.symbol}`
      );
      significantChangesDetected = true;

      if (nepseNotification) {
        nepseLogger.info(
          `Notification triggered for symbol: ${currentItem.symbol} - New Item`
        );
        await sendSupplyDemandNotification(currentItem);
      }
    } else {
      const significantChangeInBuyQuantity = calculatePercentageDifference(
        previousItem.totalBuyQuantity,
        currentItem.totalBuyQuantity
      );
      const significantChangeInSellQuantity = calculatePercentageDifference(
        previousItem.totalSellQuantity,
        currentItem.totalSellQuantity
      );

      if (significantChangeInBuyQuantity || significantChangeInSellQuantity) {
        nepseLogger.info(
          `Significant change detected for symbol: ${currentItem.symbol}`
        );
        significantChangesDetected = true;

        if (nepseNotification) {
          nepseLogger.info(
            `Notification triggered for symbol: ${currentItem.symbol} - Updated Item`
          );
          await sendSupplyDemandNotification(currentItem);
        }
      }
...
const mergeBuySellData = async (item, side, matchingList) => {
  ...
  modifiedItem.buyToSellOrderRatio = parseFloat(
    (modifiedItem.totalBuyOrder / modifiedItem.totalSellOrder).toFixed(1)
  );
  modifiedItem.buyToSellQuantityRatio = parseFloat(
    (modifiedItem.totalBuyQuantity / modifiedItem.totalSellQuantity).toFixed(1)
  );
...
export async function SupplyDemandData(refresh = false) {
  ...
  try {
    if (!refresh || isNepseOpen === false) {
      ...
      const cachedHighestQuantityperOrder = await fetchFromCache(
        "highestQuantityperOrder"
      );

      if (
        cachedHighestSupply !== null &&
        cachedHighestDemand !== null &&
        cachedHighestQuantityperOrder !== null
      ) {
        return {
    ...
    const combineAndSortTopItems = async (highestDemand, highestSupply) => {
      const combinedList = [
        ...(await Promise.all(
          highestDemand.map(
            async (item) => await mergeBuySellData(item, "Demand", supplyList)
          )
        )),
        ...(await Promise.all(
          highestSupply.map(
            async (item) => await mergeBuySellData(item, "Supply", demandList)
          )
        )),
      ];
...
export const topgainersShare = async (refresh) => {
  ...
  try {
    ...
    if (!Array.isArray(data) && cachedData != null) {
      await switchServer();
      assetLogger.error(
        "Invalid data received from the API in topgainersShare."
      );
      return cachedData;
    }
...
export const topLosersShare = async (refresh) => {
  ...
  try {
    ...
    if (!Array.isArray(data) && cachedData != null) {
      await switchServer();
      assetLogger.error(
        "Invalid data received from the API in topLosersShare."
      );
      return cachedData;
    }
...
export const topTurnoversShare = async (refresh) => {
  ...
  try {
    ...
    if (!Array.isArray(data) && cachedData != null) {
      await switchServer();
      assetLogger.error(
        "Invalid data received from the API in topTurnoversShare"
      );
      return cachedData;
    }
...
export const topTradedShares = async (refresh) => {
  ...
  try {
    ...
    if (!Array.isArray(data) && cachedData != null) {
      await switchServer();
      assetLogger.error(
        "Invalid data received from the API in TopTenTradeScrips."
      );
      return cachedData;
    }
...
export const topTransactions = async (refresh) => {
  ...
  try {
    ...
    if (!Array.isArray(data) && cachedData != null) {
      await switchServer();
      assetLogger.error(
        "Invalid data received from the API in toptransaction. Switching server."
      );
      return cachedData;
    }
...
export async function fetchAvailableNepseSymbol(filterdeben = true, refresh) {
  ...
  try {
    const response = await fetch(
      "https://api.npstocks.com/tv/tv/search?limit=30&query=&type=&exchange=",
      {
        headers: {
          accept: "*/*",
    ...
    if (filterdeben) {
      symbols = symbols.filter(
        (symbol) => !/\d/.test(symbol) && !symbol.includes("/")
      );
    }
...
export const fetchFunctionforNepseAlphaORSystemxlite = async (
  symbolIndex,
  timeFrame,
  fromEpochTime,
  currentEpochTime,
  ...
  try {
    ...
    assetLogger.error(
      "Fetching data from npstocks.com failed. Trying nepsealpha.com"
    );
