...
export const wipeCachesAndRefreshData = async () => {
  try {
    const lastDataRefreshTime = await getLastDataRefreshTime();
    console.log(`Last Refreshed Data Date and Time: ${lastDataRefreshTime || 'N/A'}`);
    const now = new Date();
    ...
    if (
      (isWeekday && currentHour >= 11 && currentHour < 15) ||
      (isWeekday && currentHour >= 15 && now - lastDataRefreshTime > fifteenMinutesInMilliseconds) ||
      (isWeekday && now - lastDataRefreshTime > oneDayInMilliseconds)
    ) {
      console.log('Wiping cache and refreshing data.');
      console.log(`Last Refreshed Data Date and Time: ${lastDataRefreshTime || 'N/A'}`);

      // Existing code for wiping caches
      await Promise.all([
        storage.removeItem(CACHE_KEY_ALL_ASSET_NAMES),
        storage.removeItem(CACHE_KEY_SINGLE_ASSET),
        storage.removeItem(CACHE_KEY),
      ...
      // Save the current timestamp as the last data refresh time
      await saveLastDataRefreshTime(now);
