...
import {
  FindHighestContractAmount,
  FindHighestContractQuantity,
  GetFloorsheet,
} from "../server/floorsheetServer.js";

//import nepseFloorsheet from "../server/nepse_server/test.js";
...
export async function checkIsNepseOpen() {
  ...
  try {
    ...
    if (response.status === 200) {
      return await handlecheckIsNepseOpenResponse(
        response,
...
async function handlecheckIsNepseOpenResponse(response, apiUrl) {
  if (response.data.isOpen === "CLOSE") {
    ...
    response.data.isOpen === "OPEN" ||
    response.data.isOpen === "Pre Open"
  ) {
  ...
  try {
    ...
    if (newIndexData) {
      ...
      const title = `Nepse Index ${newIndexData.change} ${
        newIndexData.change > 0 ? "Up" : "Down"
      } ${asOf}`;
      const body = `Nepse Index: ${newIndexData.close} • Point change ${
        newIndexData.change
      } • Percentage Change ${newIndexData.percentageChange}% • High ${
        newIndexData.high
      } • Low ${newIndexData.low} • Turnover ${formatTurnover(
        newIndexData.turnover
      )} `;
    ...
    nepseLogger.info(
      `Refresh Successful at ${new Date().toLocaleTimeString("en-US", {
        hour12: false,
        hour: "2-digit",
        minute: "2-digit",
        second: "2-digit",
      })}, Data as of ${formatTimeTo12Hour(newIndexData.time)}`
    );
  } catch (error) {
...
async function fetchNepseFloorsheet() {
  ...
  try {
    ...
    nepseLogger.error(
      `Error in fetching and processing floorsheet data: ${error.message}`
    );
  }
}
...
export default async function initializeRefreshMechanism() {
  try {
    ...
    nepseLogger.error(
      `Error initializing Nepse refresh mechanism ${error.message}`
    );
  }
}
